...
for reinforcement_type in reinforcement_types:
    ...
    for reinforcement in reinforcements:
        ...
        # Ausgabe aller möglichen Attribute und Werte (einschließlich benutzerdefinierter Attribute)
        print("\n-- Zusätzliche Attribute (Benutzerdefiniert) --")
        additional_attributes = dir(reinforcement)
        for attr in additional_attributes:
            if not attr.startswith('_') and not callable(getattr(reinforcement, attr)):
                value = getattr(reinforcement, attr)
                print(f"{attr}: {value}")
